...
router.post('/register', (req, res) => {
  ...
  const info = db
    ...
    .run(email.toLowerCase(), passwordHash, name, phone || null);

  const customer = { id: info.lastInsertRowid, email: email.toLowerCase() };
  res.status(201).json({
...
router.get('/me', requireCustomerAuth, (req, res) => {
  const customer = db.prepare('SELECT id, email, name, phone, loyalty_points FROM customers WHERE id = ?').get(req.customerId);
